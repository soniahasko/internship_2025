...
'''
loop through every crystal in the multiphase list of crystals
'''
...
for i in range(len(all_crystals)):

    crystal = all_crystals[i] # choose a crystal to simulate
    cif_file = f'cif_files/{crystal}.cif'
    all_xtls.append(dif.Crystal(cif_file)) # load in the cif file
    ...
    all_intens.append(noisy_signal)
    all_binary.append(binary_peaks)
...
def vis_combined_separate(pattern=0):
    plt.figure()
    plt.plot(tths[pattern], combined_intens[pattern], color='orange')
    plt.show()
